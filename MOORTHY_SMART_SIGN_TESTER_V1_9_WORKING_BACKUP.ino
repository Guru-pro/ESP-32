/*
  ============================================================
       MOORTHY SMART SIGN TESTER V2.0
       ESP32 + WS2811 + 3-LED MODULE + PIXEL + CX1903
       VOLTAGE + CURRENT + POWER FRAMEWORK
  ============================================================
  ...
  1. WS2811
     7 ADDRESSABLE SECTIONS
     3 PHYSICAL LEDs / SECTION

  2. WS2811 3-LED MODULE
     7 ADDRESSABLE MODULES
     3 PHYSICAL LEDs / MODULE

  3. ADDRESSABLE PIXEL
     7 PIXELS
     1 PHYSICAL LED / PIXEL
...
CLEDController *wsController = nullptr;
...
// CURRENT SENSOR IS NOT ENABLED YET.
// We will enable this after the 10K + 20K
// resistor divider is installed and tested.

bool currentSensorEnabled = false;

// ACS712 30A nominal sensitivity.
// This will be calibrated later.
float currentZeroVoltage = 2.50;
float currentSensitivity = 0.066;

// Current divider:
// ACS712 OUT -> 10K -> GPIO35
// GPIO35 -> 20K -> GND
//
// We will account for this divider later.
...
{
  return sectionCount * physicalLEDsPerSection;
}
...
void loadProfile()
{
  if (currentLEDType == LED_WS2811)
  {
    sectionCount = wsSections;
    physicalLEDsPerSection = wsLedsPerSection;
  }

  else if (currentLEDType == LED_WS2811_MODULE)
  {
    sectionCount = moduleSections;
    physicalLEDsPerSection = moduleLedsPerSection;
  }

  else if (currentLEDType == LED_PIXEL)
  {
    sectionCount = pixelSections;
    physicalLEDsPerSection = pixelLedsPerSection;
  }

  else
  {
    sectionCount = cxSections;
    physicalLEDsPerSection = cxLedsPerSection;
  }
...
void saveProfile()
{
  if (currentLEDType == LED_WS2811)
  {
    wsSections = sectionCount;
    wsLedsPerSection = physicalLEDsPerSection;
  }

  else if (currentLEDType == LED_WS2811_MODULE)
  {
    moduleSections = sectionCount;
    moduleLedsPerSection = physicalLEDsPerSection;
  }

  else if (currentLEDType == LED_PIXEL)
  {
    pixelSections = sectionCount;
    pixelLedsPerSection = physicalLEDsPerSection;
  }

  else
  {
    cxSections = sectionCount;
    cxLedsPerSection = physicalLEDsPerSection;
  }
}
...
{
  if (
    currentLEDType == LED_WS2811 ||
    currentLEDType == LED_WS2811_MODULE ||
    currentLEDType == LED_PIXEL
  )
  {
  ...
  Serial.println(getLEDTypeName());

  Serial.print("PROTOCOL : ");
  Serial.println(getProtocolName());

  Serial.print("ORDER    : ");
  Serial.println(getColourOrder());

  Serial.print("VOLTAGE  : ");
  Serial.println(getProfileVoltage());
...
void chaseEffect()
{
  ...
  for (int tail = 0; tail < 4; tail++)
  {
    ...
    leds[pos % sectionCount] =
      colours[
        (chasePosition + tail) % 7
      ];
  }
...
float readCurrent()
{
  // Current sensor is intentionally
  // disabled until the resistor divider
  // and ACS712 are installed.
...
void handleSettings()
{
  ...
  if (server.hasArg("leds"))
  {
    ...
    if (physicalLEDsPerSection > 20)
      physicalLEDsPerSection = 20;
  }
